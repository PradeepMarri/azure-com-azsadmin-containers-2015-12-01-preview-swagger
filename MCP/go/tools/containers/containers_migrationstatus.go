...
func Containers_migrationstatusHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourcegroups/%s/providers/Microsoft.Storage.Admin/farms/%s/shares/operationresults/%s%s", cfg.BaseURL, subscriptionId, resourceGroupName, farmId, operationId, queryString)
...
func CreateContainers_migrationstatusTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_subscriptions_subscriptionId_resourcegroups_resourceGroupName_providers_Microsoft_Storage_Admin_farms_farmId_shares_operationresults_operationId",
		mcp.WithDescription("Returns the status of a container migration job."),
		...
		mcp.WithString("operationId", mcp.Required(), mcp.Description("Operation Id.")),
		mcp.WithString("api-version", mcp.Required(), mcp.Description("REST Api Version.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Containers_migrationstatusHandler(cfg),
	}
}
