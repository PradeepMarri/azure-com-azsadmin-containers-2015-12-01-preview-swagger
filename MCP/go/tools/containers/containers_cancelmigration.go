...
func Containers_cancelmigrationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateContainers_cancelmigrationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_subscriptions_subscriptionId_resourcegroups_resourceGroupName_providers_Microsoft_Storage_Admin_farms_farmId_shares_operationresults_operationId",
		mcp.WithDescription("Cancel a container migration job."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Containers_cancelmigrationHandler(cfg),
	}
}
